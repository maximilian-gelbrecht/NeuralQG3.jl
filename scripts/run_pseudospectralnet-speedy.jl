...
if length(ARGS) > 1
    ...
    psn_hyperpars = PSNHyperparameters(N_layers=3, additional_knowledge=true, N_Nodes=50, unet_channels=[24,48,48,96], znorm=false, unet_kernel=(3,3), unet_convblock=NeuralQG3.LongLatConvBlock, activation=NNlib.swish)
    other_hyperpars = NeuralQG3.training_hyperpars(N_epoch_ad=0, N_epoch_sciml=10, N_epoch_offset=500, τ_max=6, data_length=200f0, γ=1f-5)
...
DATA_MODEL = PRETRAIN ? :qg3 : :speedy
(t, q) = NeuralQG3.get_data(DATA_MODEL, qg3p, PRECOMPUTE; S=S, DT=DT, q_0=q_0, period_data=Day(220), t_save_length=data_length)
...
neural_de, ps, st = NeuralQG3.load_psn_ad(qg3p, psn_hyperpars, DT, N_batch; device=DEV, S=S)
...
η_schedule = try
    ...
    SinExp(l0=η,l1=1f-7,period=40,decay=0.995f0) # new syntax
end
...
if SETUP_SCIML 
    ...
    println("Final Forecast:")
    println(grid_forecast(neural_de_sciml, ps, st))
    println("--------------")
end 
